//! Example demonstrating PNG rendering
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let path = std::path::Path::new("./trash/output.png");
    image.save(path, &mut png)?;

    println!("PNG file created at: {}", path.display());
...
#[cfg(not(feature = "png"))]
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("PNG feature is not enabled");
    Ok(())
}
